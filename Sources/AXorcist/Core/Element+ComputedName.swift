...
extension Element {
    ...
    @MainActor
    public func computedName() -> String? { // Removed async
        // Prioritize specific, descriptive attributes first
        if let title = self.title(), !title.isEmpty { // title() will become sync
            GlobalAXLogger.shared.log(AXLogEntry(level: .debug, message: "ComputedName: Using AXTitle '\(title)' for \(self.briefDescription(option: .raw))")) // Removed await
            return title
        }
        if let value = self.value() as? String, !value.isEmpty { // value() will become sync
            ...
            GlobalAXLogger.shared.log(AXLogEntry(level: .debug, message: "ComputedName: Using AXValue '\(truncatedValue)' (truncated) for \(self.briefDescription(option: .raw))")) // Removed await
            return truncatedValue
        }
        if let identifier = self.identifier(), !identifier.isEmpty { // identifier() will become sync
            GlobalAXLogger.shared.log(AXLogEntry(level: .debug, message: "ComputedName: Using AXIdentifier '\(identifier)' for \(self.briefDescription(option: .raw))")) // Removed await
            return identifier
        }
        if let desc = self.descriptionText(), !desc.isEmpty { // descriptionText() will become sync
            GlobalAXLogger.shared.log(AXLogEntry(level: .debug, message: "ComputedName: Using AXDescription '\(desc)' for \(self.briefDescription(option: .raw))")) // Removed await
            return desc
        }
        if let help = self.help(), !help.isEmpty { // help() will become sync
            GlobalAXLogger.shared.log(AXLogEntry(level: .debug, message: "ComputedName: Using AXHelp '\(help)' for \(self.briefDescription(option: .raw))")) // Removed await
            return help
        }
        ...
        if let role = self.role(), !role.isEmpty { // role() will become sync
            ...
            GlobalAXLogger.shared.log(AXLogEntry(level: .debug, message: "ComputedName: Falling back to AXRole '\(cleanRole)' for \(self.briefDescription(option: .raw))")) // Removed await
            return cleanRole
        }

...
public func handleQuery(cmd: CommandEnvelope, isDebugLoggingEnabled: Bool) async throws -> QueryResponse {
    ...
    guard let locator = cmd.locator else {
        return QueryResponse(
            command_id: cmd.command_id,
            attributes: nil,
            error: "Locator not provided in command.",
            debug_logs: isDebugLoggingEnabled ? handlerLogs : nil
        )
    }
    ...
    if isAppOnlyLocator {
        ...
        if let rootPathHint = locator.root_element_path_hint, !rootPathHint.isEmpty {
            ...
            dLog(
                "Searching with locator within container found by root_element_path_hint: \(searchStartElementForLocator.briefDescription(option: .default, isDebugLoggingEnabled: isDebugLoggingEnabled, currentDebugLogs: &handlerLogs))"
            )
        } else {
            searchStartElementForLocator = effectiveElement
            dLog(
                "Searching with locator from element (determined by main path_hint or app root): \(searchStartElementForLocator.briefDescription(option: .default, isDebugLoggingEnabled: isDebugLoggingEnabled, currentDebugLogs: &handlerLogs))"
            )
        }

        let finalSearchTarget = (cmd.path_hint != nil && !cmd.path_hint!.isEmpty) ? effectiveElement :
